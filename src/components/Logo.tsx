...
export default function Logo({
  ...
  light?: boolean;
  priority?: boolean;
  /** En-tête : entre 1024 et 1280 px seul le logo reste ; au-delà « Permis Maison » avec le seul « 100 % en ligne », le menu manque de place. */
  compact?: boolean;
}) {
  return (
    <span className={`inline-flex shrink-0 items-center gap-3 sm:gap-4 ${light ? "text-paper" : "text-ink"} ${className}`}>
      ...
      <span className={`leading-none ${compact ? "lg:hidden xl:block" : ""}`}>
        ...
        <span className={`mt-1 text-[0.5rem] sm:text-[0.58rem] font-bold uppercase tracking-[0.14em] whitespace-nowrap block ${light ? "text-brass-2" : "text-ink-2"}`}>
          100 % en ligne
          <span className={compact ? "lg:hidden" : ""}>
